...
**[Live demos](https://ajfriend.com/ajglobe/examples/)** — every example runs on
GitHub Pages. Small data (~2MB: H3 r1/r2, mu3 r0–r4, ivea7h r1–r3) is checked in,
so the demos and fresh clones work out of the box; only the r5/r6 torture-test
binaries (tens of MB) are local-only. To run locally:

```sh
uv run -m http.server 8080 -d .   # then open /examples/
```

`examples/dggs-globe.html` renders every ivea7h cell — the **r5/r6 torture tests**
(168k / 1.18M cells) need the local binaries in `examples/data/` (generated by
skar_py's `scripts/dggs_cache/web/build_ivea7h_full_ar.py`); r1–r3 work anywhere.
...
To regenerate its data (H3 is native; the AR needs skar from
...
5–6 local-only); hover a cell for its id (+ pentagon tag) and AR. Regenerate with
`uv run scripts/gen_mu3_geom.py` (edit `RES_LIST`), then the same `gen_cells_ar.py`
pass as above (edit its `NAMES`).

`examples/reference-detail.html` compares coastline/border/city **detail levels**
(110m/50m/10m) plus graticule and small-circle rings — no local data at all
(Natural Earth via CDN).
